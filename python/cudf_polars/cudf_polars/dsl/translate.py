...
class Translator:
    ...
    def translate_ir(self, *, n: int | None = None) -> ir.IR:
        ...
        if (version := self.visitor.version()) >= (3, 0):
        ...
        with ctx:
            ...
            try:
                ...
                return ir.ErrorNode(polars_schema, str(e))
            if any(
...
def _(
    ...
    return ir.Slice(
        schema, translator.translate_ir(n=node.input), node.offset, node.len
    )
...
def _(
    ...
    return ir.Union(
        schema, [translator.translate_ir(n=n) for n in node.inputs], node.options
    )
